...
while len(answer_list) < 50:
    ...
    if answer_state == 'Exit':
        missing = [state for state in states_csv['state'].to_list() if state not in answer_list]
        output = pandas.DataFrame(missing)
        output.columns = ['state']
        output.to_csv('./missed_states.csv')
        break
    elif answer_state in states_csv['state'].to_list() and answer_state not in answer_list:
